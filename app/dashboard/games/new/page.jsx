...
export default function VintageGameGeneratorPage() {
  ...
  const generateGame = async () => {
    ...
    // Validate input
    if (!gameRequest.trim()) {
      setError('Please describe the aptos minigames black and white game you want to create');
      return;
    }
    ...
    try {
      const response = await axios.post('/api/new', { 
        prompt: `Create a aptos minigames black and white game: ${gameRequest}`
      }, {
        headers: {
            ...
            <Card className="w-full h-full bg-gray-800 border-gray-700">
              ...
              <CardContent>
                <Textarea
                  ...
                  placeholder="Describe a Aptos Minihub with Ai agent (e.g., 'tic-tac-toe', 'Space Invaders', 'Pac-Man')"
                />
                <Button
                  ...
                  {gameComponent || (
                    <div className="flex items-center justify-center h-full text-gray-400">
                      <p className="text-center">
                        Generated Aptos minigame will appear here
                      </p>
                    </div>
                  )}
                </div>
